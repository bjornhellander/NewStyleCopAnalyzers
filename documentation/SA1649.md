...
Files with a `.cshtml.cs` extension are ignored. The name of a Razor page's code-behind file is determined by its
sibling `.cshtml` page rather than by the type it contains, so renaming it to match the type would break the page.

...
The `break` and `continue` keywords should always be followed by a space when they name a label (`break label;` /
`continue label;`), unless there is no label, in which case there should be no space between the keyword and the
semicolon.

The `unsafe` keyword should not be followed by any space when it is part of an `unsafe` expression (`unsafe(expression)`).
Other uses of the `unsafe` keyword, such as the `unsafe` modifier or the `unsafe` statement, are not covered by this rule.
